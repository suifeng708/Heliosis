...
public class Config {
    ...
    public static final File DIRECTORY = new File(Myau.CONFIG_DIR);
    private static final File LEGACY_DIRECTORY = new File(Myau.LEGACY_CONFIG_DIR);
    private static final File LEGACY_OPENMYAU_DIRECTORY = new File(Myau.LEGACY_OPENMYAU_CONFIG_DIR);
    public String name;
    public File file;
    ...
    public Config(String name, boolean newConfig) {
        ...
        this.file = resolveFile(String.format("%s.json", this.name));
        try {
    ...
    public static String getDirectoryPath() {
        return Myau.CONFIG_DIR;
    }

    public static File resolveFile(String fileName) {
        File file = new File(getDirectory(), fileName);
        if (!file.exists()) {
            migrateLegacyFile(new File(LEGACY_DIRECTORY, fileName), file);
        }
        if (!file.exists()) {
            migrateLegacyFile(new File(LEGACY_OPENMYAU_DIRECTORY, fileName), file);
        }
        return file;
    }

    private static void migrateLegacyFile(File legacy, File target) {
        if (!legacy.exists()) {
            return;
        }
        try {
