    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            ...
            switch (sub) {
                ...
                case "list":
                    try {
                        File[] configs = Config.getDirectory().listFiles(FILE_FILTER);
                        if (configs == null) {
                            throw new Exception();
                        }
                        if (configs.length == 0) {
                            ChatUtil.sendFormatted(String.format("%sNo configs found (&o%s&r)&r", Myau.clientName, Config.getDirectoryPath()));
                        }
                        ...
                        ChatUtil.sendFormatted(String.format("%sFailed to read (&o%s&r)&r", Myau.clientName, Config.getDirectoryPath()));
                    }
                    return;
                case "f":
                case "folder":
                ...
                case "directory":
                    try {
                        Desktop.getDesktop().open(Config.getDirectory());
                    } catch (Exception e) {
                        ChatUtil.sendFormatted(String.format("%sFailed to open (&o%s&r)&r", Myau.clientName, Config.getDirectoryPath()));
                    }
                    return;
                default:
